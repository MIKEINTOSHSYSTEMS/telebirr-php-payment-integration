...
class ApplyFabricToken
{
    ...
    public function __construct($config, $logger = null, $apiLogger = null)
    {
    ...
    public function applyToken()
    {
        ...
        try {
            ...
            if ($this->apiLogger) {
                $this->apiLogger->log($url, $method, $payload, $response, $httpCode);
            }
            ...
            // Check for errors
            if ($httpCode != 200) {
                $errorMsg = isset($result['errorMsg']) ? $result['errorMsg'] : (isset($result['message']) ? $result['message'] : 'Unknown error');
